...
import {
  ...
  calculateYTD,
} from "../models/payrollCalculations.js";
...
export const runPayroll = async (req, res) => {
  ...
  if (month === undefined || !year) {
    return res
      .status(400)
      .json({ error: "month (0-11) and year are required" });
  }
...
export const getPayslip = async (req, res) => {
  ...
  try {
    ...
    const figures = calculatePayslipFigures(
      employeeData,
      new Date(record.month).getMonth(),
    );
...
export const getYTD = async (req, res) => {
  ...
  const throughMonth =
    req.query.throughMonth !== undefined
      ? Number(req.query.throughMonth)
      : new Date().getMonth();
...
export const updatePayroll = async (req, res) => {
  try {
    ...
    const [existing] = await db.query("SELECT * FROM payroll WHERE id = ?", [
      id,
    ]);
